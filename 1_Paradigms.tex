...
delFst _ [] = []
delFst a (x:xs) 	| a == x  	= xs
				| otherwise = delFst a xs 

-- drp :: Int -> [a] -> [a]
-- returns the list xs with the first n elements removed
drp 0 xs = xs
drp _ [] = []
...
-- halve :: [a] -> ([a],[a])
-- splits a list of even length into two equal halves.
halve xs = (tke n xs, drp n xs)
